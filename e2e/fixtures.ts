...
function goSource(relative: string): string {
  ...
  try {
    ...
    throw new Error(
      `e2e: cannot read ${path}, which is where the fixture identifiers are declared`,
      { cause },
    );
  }
}
...
function goString(source: string, relative: string, name: string): string {
  ...
  if (!found) {
    throw new Error(
      `e2e: ${relative} no longer declares ${name}, so the browser gate is addressing a record nobody seeds`,
    );
  }
  return found[1];
}
...
function segmentOf(kind: string): string {
  const row = new RegExp(
    `\\{\\s*kind:\\s*${kind}\\s*,[^}]*segment:\\s*"([^"]+)"`,
  ).exec(kinds);
  if (!row) {
    throw new Error(
      `e2e: ${kindGo} has no row for ${kind}, so its pages have no address`,
    );
  }
  return row[1];
}
...
function enumOf(kind: string): string {
  const declared = new RegExp(`\\b${kind}\\s+Kind\\s*=\\s*"([^"]+)"`).exec(
    kinds,
  );
  if (!declared) {
...
function goIdentifier(name: string, before: number): string {
  ...
  for (
    let found = declarations.exec(routes);
    found !== null;
    found = declarations.exec(routes)
  ) {
  ...
  if (declared === null) {
    throw new Error(
      `e2e: ${routesGo} declares no ${name} above the row that uses it`,
    );
  }
...
function goRoutePath(opID: string): string {
  ...
  if (!row) {
    throw new Error(
      `e2e: ${routesGo} has no ${opID} route, so the gate is driving an operation nobody serves`,
    );
  }
...
function goPageTarget(opID: string): PageTarget {
  ...
  if (
    ...
    throw new Error(
      `e2e: ${opID}'s landmark role ${role} is not one the gate can address`,
    );
  }

  return {
    path: goExpression(row[3], row.index),
    landmark: { role, name: row[2] },
  };
}
...
export const fixtures = {
  ...
  settingsLandmarks: {
    ...
    deleteAccount: goString(
      settingsView,
      settingsViewGo,
      "DeleteAccountLandmark",
    ),
  },
  ...
  deleteConfirmationPhrase: goString(
    recovery,
    recoveryGo,
  ...
  // US6's demonstration case (T142, T150): the one seeded treatment carrying
  // both a Condition relation and an attached course medication.
  treatment: {
    listPath: `/${segmentOf("Treatment")}`,
    detailPath: (id: string) => `/${segmentOf("Treatment")}/${id}`,
    // seededID carries neither relation; linkedID carries both (T142, T150).
    seededID: goString(identifiers, fixturesGo, "TreatmentNameOnlyID"),
    linkedID: goString(identifiers, fixturesGo, "LinkedTreatmentID"),
    linkedMedicationID: goString(
      identifiers,
      fixturesGo,
      "LinkedTreatmentMedicationID",
    ),
  },
  ...
  resetPasswordPath: (token: string) =>
    goRoutePath("resetPasswordPage").replace(
      `{${goString(accountPages, accountPageGo, "PathToken")}}`,
      token,
    ),
  ...
  title: (page: string) =>
    page === ""
      ...
      : page +
        goString(shell, shellGo, "SuffixSeparator") +
        goString(shell, shellGo, "ProductName"),
} as const;
