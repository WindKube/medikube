...
- [ ] T002 [P] Add `Labs.MaxSeriesPoints` (`MEDIGO_LABS_MAX_SERIES_POINTS`, default 500) to `internal/config/config.go`, and set the `Files.AllowedMIME` default to `application/pdf,image/jpeg,image/png,image/webp,image/heic,image/heif,image/tiff,image/gif,text/plain` and `Files.MaxUploadBytes` to 33554432 per research D-14/D-15
- [ ] T003 [P] Extend `internal/config/config_test.go` with the new defaults, the allowed-MIME list parse (comma list, trimmed, lower-cased, rejecting an empty entry), and a case asserting `MEDIGO_LABS_MAX_SERIES_POINTS=0` is rejected at boot
- [ ] T004 [P] Add the `labs-never-convert` `depguard` rule to `.golangci.yml`: files under `internal/domain/labs/**`, `internal/service/lab*/**` and `internal/store/lab*/**` may not import `medigo/internal/domain/clinical/units` (research D-09)
...
- [ ] T006 [P] Add `task purge` (wrapping `medigo purge`) to `Taskfile.yaml`, and register `e2e/specs/labs.spec.ts`, `e2e/specs/trends.spec.ts` and `e2e/specs/documents.spec.ts` in the Playwright project config
...
- [ ] T009 [P] Confirm `/Users/krzysztof.wiatrzyk/private/monorepo/.dockerignore` still excludes `medigo/pb_data/` (which now holds uploaded documents) and `medigo/**/*_templ.go`, and that `medigo` is in the `build-image.yaml` project options
...
- [ ] T043 [P] Write a failing test in `internal/httproute/registry_test.go` for `KindPageAction`: such a route appears in `medigo routes`, is excluded from the OpenAPI input set, has no landmark, and **must** declare a non-empty covering Playwright spec path or `Register` panics at startup (research D-25)
...
- [ ] T086 [P] [US2] Write failing tests proving `disposition=inline` is honoured only for the compile-time inline-safe set, that any other type is served as `attachment` rather than refused (FR-057), and that **widening `MEDIGO_FILES_ALLOWED_MIME` does not widen what is inlined** (FR-058)
...
- [ ] T102 [US2] Implement `internal/store/filestore/filestore.go` and `thumb.go` over `app.NewFilesystem()`, plus the in-memory fake in `internal/service/attachment/attachmenttest/filestorefake.go`; both pass the contract suite. **This is the only package in MediGo permitted to move bytes**
...
- [ ] T113 [US2] Register `medigo_attachment_maintenance` in `internal/platform/pb/cron.go`
- [ ] T114 [US2] Implement `internal/cli/purge.go` — `medigo purge` runs the same maintenance function once
...
- [ ] T119 [US2] Register `medigo_files_uploads_total{outcome,reason}`, `medigo_files_bytes_total` and `medigo_files_serve_duration_seconds{disposition,outcome}` in `internal/obs/`, with a test asserting **no** label value can be a file name, patient id or attachment id
...
- [ ] T184 [P] Update the project README's route table and the operator notes covering `MEDIGO_FILES_*`, `MEDIGO_RETENTION_TRASH_DAYS` and `MEDIGO_LABS_MAX_SERIES_POINTS`, including the `text/csv` sniffing note from `quickstart.md` §1
...
- Verify a test fails before implementing against it. A red-to-green transition that was never red
  proves nothing.
- Commit after each task or logical group, `feat(medigo): …` per the house convention.
