...
- [x] T201 Confirm `e2e/routes.gate.spec.ts` derives its route list from `medikube routes` and **fails** when a page route is added without a smoke case, **and when a status-view catalogue entry has no `SmokeVariants` entry**; prove both by temporarily adding a bare route and a bare catalogue entry (SC-015, L2)
