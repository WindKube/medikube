...
- [x] T030 Implement the write-side indexer `internal/service/search/indexer.go` and its `Indexer` port declared in `internal/service/search/ports.go`; wire it into `internal/records/register.go` so every `Register` call binds it. The read side is US8. **Wiring deferred**: binding one indexer per `Register` call needs the `SearchFields` function T019-T021 add to `Registration` — the indexer and its port are complete and unit-tested against a fake repository, but nothing calls them yet.
