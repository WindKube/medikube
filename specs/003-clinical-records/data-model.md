...
4. **All five API rules (`ListRule`, `ViewRule`, `CreateRule`, `UpdateRule`, `DeleteRule`) are
   `nil`** on every collection here. Superuser-only. Asserted at boot; MediGo refuses to start
...
running the migrations against a clean database plus `medigo seed`, and committed — every
