...
- [x] T126 [P] [US5] Integration test asserting that deleting a practitioner referenced by `patients.primary_practitioner` and by `medications.practitioner` leaves both records alive with the reference cleared, and that the number of records modified equals the `usage.records` reported beforehand (FR-040, US5-5) — in `internal/store/practitioner/delete_unset_integration_test.go`
