...
import (
	...
	service "medikube/internal/service/allergy"
	"medikube/internal/store"
)

// Repo is the allergy repository service.Repository declares, against
// PocketBase. Get, Update and Delete are unscoped by patient: the service
// resolves and authorizes the patient from the row itself (research D-13).
type Repo struct {
	app     core.App
...
func New(app core.App, cursors *store.CursorCodec) (*Repo, error) {
	...
	if len(missing) > 0 {
		return nil, fmt.Errorf("allergy: the repository is wired with no %s", missing[0])
	}

	return &Repo{app: app, cursors: cursors, schema: allergySchema()}, nil
}

func (r *Repo) List(ctx context.Context, patientID string, query service.Query) (domain.Page[clinical.Allergy], error) {
	var empty domain.Page[clinical.Allergy]
	...
	if queryErr := built.Apply(r.app.RecordQuery(kind.Allergy.Collection())).
		WithContext(ctx).All(&records); queryErr != nil {
		return empty, fmt.Errorf("listing %s: %w", kind.Allergy, queryErr)
	}
	...
	items := make([]clinical.Allergy, 0, len(records))

	for _, record := range records {
		entity, mapErr := recordFromAllergy(record)
		if mapErr != nil {
			return empty, mapErr
		}
...
func (r *Repo) narrowing(patientID string, query service.Query) []store.Condition {
	...
	if query.Search != "" {
		conditions = append(conditions, store.Contains(fieldAllergen, query.Search))
	}
	...
	if len(query.Severities) > 0 {
		conditions = append(conditions, store.OneOf(fieldSeverity, stringsOfSeverity(query.Severities)...))
	}
...
func stringsOfSeverity(values []clinical.Severity) []string { return stringsOf(values) }
...
func (r *Repo) count(ctx context.Context, conditions []store.Condition) (int, error) {
	...
	counting := r.app.RecordQuery(kind.Allergy.Collection()).Select("count(*)")
	if built.Where != nil {
	...
	if rowErr := counting.WithContext(ctx).Row(&total); rowErr != nil {
		return 0, fmt.Errorf("counting %s: %w", kind.Allergy, rowErr)
	}
...
func (r *Repo) Get(ctx context.Context, id string) (clinical.Allergy, error) {
	record, err := r.byID(ctx, r.app, id)
	if err != nil {
		return clinical.Allergy{}, err
	}

	return recordFromAllergy(record)
}

func (r *Repo) Create(ctx context.Context, entity clinical.Allergy) (clinical.Allergy, error) {
	...
	if mapErr := allergyToRecord(record, entity); mapErr != nil {
		return clinical.Allergy{}, mapErr
	}

	if saveErr := r.app.SaveWithContext(ctx, record); saveErr != nil {
		return clinical.Allergy{}, fmt.Errorf("creating a %s: %w", kind.Allergy, saveErr)
	}

	return recordFromAllergy(record)
}

func (r *Repo) Update(ctx context.Context, entity clinical.Allergy, expectedVersion string) (clinical.Allergy, error) {
	var updated clinical.Allergy
	...
	write := func(txApp core.App) error {
		...
		if saveErr := txApp.SaveWithContext(ctx, record); saveErr != nil {
			return fmt.Errorf("updating %s %s: %w", kind.Allergy, entity.ID, saveErr)
		}

		mapped, mapErr := recordFromAllergy(record)
		if mapErr != nil {
			return mapErr
		}
...
func (r *Repo) Delete(ctx context.Context, id, expectedVersion string) error {
	return store.RunInTransaction(r.app, func(txApp core.App) error {
		...
		if deleteErr := txApp.DeleteWithContext(ctx, record); deleteErr != nil {
			return fmt.Errorf("deleting %s %s: %w", kind.Allergy, id, deleteErr)
		}
...
func (r *Repo) byID(ctx context.Context, app core.App, id string) (*core.Record, error) {
	...
	if queryErr := built.Apply(app.RecordQuery(kind.Allergy.Collection())).
		WithContext(ctx).All(&records); queryErr != nil {
		return nil, fmt.Errorf("reading %s %s: %w", kind.Allergy, id, queryErr)
	}

	if len(records) == 0 {
		return nil, fmt.Errorf("%s %s: %w", kind.Allergy, id, domain.ErrNotFound)
	}
...
func (r *Repo) collection(app core.App) (*core.Collection, error) {
	collection, err := app.FindCachedCollectionByNameOrId(kind.Allergy.Collection())
	if err != nil {
		return nil, fmt.Errorf("reading the %s collection: %w", kind.Allergy, err)
	}
...
func expectVersion(record *core.Record, id, expected string) error {
	...
	return fmt.Errorf("%s %s: %w", kind.Allergy, id, domain.ErrVersionMismatch)
}

func scope(patientID string) string {
	return kind.Allergy.Collection() + "\x00" + patientID
}
