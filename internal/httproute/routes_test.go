...
func TestTheTableCarriesTheFortyThreePages(t *testing.T) {
	...
	want := []struct {
		...
		},
		{
			...
			`article[name="Vaccination"]`, "/" + kind.Immunization.Segment() + "/" + seed.ImmunizationSampleID,
		},
		{
			"injuryListPage", "/" + kind.Injury.Segment(), httproute.AuthUser, `region[name="Injuries"]`,
			"/" + kind.Injury.Segment() + "?patient=" + seed.AccountAPatientSelfID,
		},
		{
		...
		{
			"familyHistoryListPage", "/" + kind.FamilyMember.Segment(), httproute.AuthUser, `region[name="Family history"]`,
			"/" + kind.FamilyMember.Segment() + "?patient=" + seed.AccountAPatientSelfID,
		},
		{
