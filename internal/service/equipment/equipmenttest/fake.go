// Package equipmenttest is the in-memory second implementation of
// equipment.Repository (Principle I), used by the service's own unit tests and
// by the kind-registration contract.
...
var errInvalidCursor = errors.New("equipmenttest: the boundary is not one this repository issued for this ordering")

const defaultLimit = 25

// Repository is the in-memory equipment.Repository.
type Repository struct {
	mu sync.Mutex

	rows  map[string]clinical.Equipment
	next  int
	clock time.Time
}

func NewRepository() *Repository {
	return &Repository{rows: make(map[string]clinical.Equipment), clock: epoch}
}
...
func (r *Repository) List(_ context.Context, patientID string, query equipment.Query) (domain.Page[clinical.Equipment], error) {
	r.mu.Lock()
	defer r.mu.Unlock()

	sortKeys := query.Sort
	if len(sortKeys) == 0 {
		sortKeys = equipment.Sorts()[:1]
	}

	matched := make([]clinical.Equipment, 0, len(r.rows))
	...
	slices.SortFunc(matched, func(left, right clinical.Equipment) int {
	...
	if query.Cursor != "" {
		...
		after := make([]clinical.Equipment, 0, len(matched))
...
func (r *Repository) Get(_ context.Context, id string) (clinical.Equipment, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Repository) Create(_ context.Context, entity clinical.Equipment) (clinical.Equipment, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	entity.ID = fmt.Sprintf("mkfakeeqp%06d", r.next)
...
func (r *Repository) Update(_ context.Context, entity clinical.Equipment, expectedVersion string) (clinical.Equipment, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if current.Version != expectedVersion {
		return clinical.Equipment{}, domain.ErrVersionMismatch
	}
...
func (r *Repository) byID(id string) (clinical.Equipment, error) {
	row, exists := r.rows[id]
	if !exists {
		return clinical.Equipment{}, fmt.Errorf("equipmenttest: no such record: %w", domain.ErrNotFound)
	}
...
func matches(row clinical.Equipment, query equipment.Query) bool {
	...
	if query.ServiceDueWithinDays != nil && len(equipment.ServiceDueBasis(row, *query.ServiceDueWithinDays)) == 0 {
		return false
	}
...
func sortValue(row clinical.Equipment, field string) string {
	switch field {
	case equipment.FieldName:
		return asciiLower(row.Name)
	case equipment.FieldPrescribedOn:
		return row.PrescribedOn.String()
	case equipment.FieldUpdated:
...
func compareValues(field string, left, right string, desc bool) int {
	if field == equipment.FieldPrescribedOn && (left == "" || right == "") {
		switch {
		case left == "" && right == "":
			return 0
		case left == "":
...
func compareRows(left, right clinical.Equipment, sortKeys []domain.SortKey) int {
...
func encodeBoundary(row clinical.Equipment, sortKeys []domain.SortKey) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("equipmenttest: sealing the boundary: %w", err)
	}
...
func compareToBoundary(row clinical.Equipment, after boundary, sortKeys []domain.SortKey) int {
...
func (Codec) Detail(entity clinical.Equipment) any {
	return &Detail{
		Summary:      Summary{ID: entity.ID, Kind: kind.Equipment.Enum(), Name: entity.Name},
		Model:        entity.Model,
		PrescribedOn: entity.PrescribedOn.String(),
	}
}

func (Codec) Draft(body any) (clinical.Equipment, error) {
	create, minted := body.(*Create)
	if !minted {
		return clinical.Equipment{}, fmt.Errorf("equipmenttest: a create was handed %T", body)
	}

	prescribedOn, _ := domain.ParseDate(create.PrescribedOn)
...
func (Codec) Patch(body any) (equipment.Patch, error) {
	patch, minted := body.(*Patch)
	if !minted {
		return equipment.Patch{}, fmt.Errorf("equipmenttest: a patch was handed %T", body)
	}

	return equipment.Patch{Name: patch.Name}, nil
}
