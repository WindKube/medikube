...
type Codec interface {
	Summary(entity clinical.FamilyMember) any
	Detail(entity clinical.FamilyMember) any
	Draft(body any) (clinical.FamilyMember, error)
	Patch(body any) (Patch, error)
}

// Adapter is records.Service for family history.
type Adapter struct {
	service *Service
	codec   Codec
}
...
func NewAdapter(service *Service, codec Codec) (*Adapter, error) {
	...
	if len(absent) > 0 {
		return nil, fmt.Errorf("familymember: the adapter is wired with no %s", joinWords(absent))
	}
...
func (a *Adapter) detail(entity clinical.FamilyMember) records.Record {
	return a.record(entity, a.codec.Detail(entity))
}

func (a *Adapter) record(entity clinical.FamilyMember, body any) records.Record {
	return records.Record{ID: entity.ID, Kind: kind.FamilyMember, PatientID: entity.PatientID, Version: entity.Version, Body: body}
}

// SeedFixtureID is the fixture `medikube seed` builds for this kind.
const SeedFixtureID = "familymember-01"

func relationships(values []string) []clinical.FamilyRelationship {
	if len(values) == 0 {
		return nil
	}

	converted := make([]clinical.FamilyRelationship, 0, len(values))
	for _, v := range values {
		converted = append(converted, clinical.FamilyRelationship(v))
	}
...
// Wiring is everything a family-history registration needs that this package
// does not own.
type Wiring struct {
...
// Register wires family history into the record registry.
func Register(registry *records.Registry, wiring Wiring) error {
	if registry == nil {
		return fmt.Errorf("familymember: there is no registry to register %s into", kind.FamilyMember)
	}
	...
	return registry.Register(records.Registration{
		Kind:       kind.FamilyMember,
		Service:    adapter,
		Schema:     schema,
		Views:      wiring.Views,
		Stream:     StreamFilter{},
		Target:     audit.TargetKindFamilyMember,
		Authorizer: wiring.Authorizer,
		Inventory: records.Inventory{
			Title:   "Family history",
			Summary: "The relatives the person has recorded, and the conditions each of them had.",
		},
...
func familyRelationshipStrings() []string {
	values := clinical.FamilyRelationships()
