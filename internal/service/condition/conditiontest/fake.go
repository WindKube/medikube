// Package conditiontest is the in-memory condition.Repository and a
// scriptable condition.Authorizer, for internal/service/condition's own unit
// tests.
...
type Repository struct {
	mu    sync.Mutex
	rows  map[string]clinical.Condition
	next  int
	clock time.Time
	calls []string
}
...
func NewRepository() *Repository {
	return &Repository{rows: make(map[string]clinical.Condition), clock: epoch}
}
...
func (r *Repository) List(_ context.Context, patientID string, query condition.Query) (domain.Page[clinical.Condition], error) {
	r.mu.Lock()
	defer r.mu.Unlock()

	r.calls = append(r.calls, "list")

	matched := make([]clinical.Condition, 0, len(r.rows))
	...
	slices.SortFunc(matched, func(left, right clinical.Condition) int {
...
func (r *Repository) Get(_ context.Context, id string) (clinical.Condition, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Repository) Create(_ context.Context, entity clinical.Condition) (clinical.Condition, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	entity.ID = fmt.Sprintf("mkfakecnd%06d", r.next)
...
func (r *Repository) Update(_ context.Context, entity clinical.Condition, expectedVersion string) (clinical.Condition, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if current.Version != expectedVersion {
		return clinical.Condition{}, domain.ErrVersionMismatch
	}
...
func (r *Repository) byID(id string) (clinical.Condition, error) {
	row, exists := r.rows[id]
	if !exists {
		return clinical.Condition{}, fmt.Errorf("conditiontest: no such record: %w", domain.ErrNotFound)
	}
