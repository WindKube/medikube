// Package injurytest is the in-memory injury.Repository and Authorizer,
// mirroring internal/service/medication/medicationtest's shape.
package injurytest
...
var errInvalidCursor = errors.New("injurytest: the boundary is not one this repository issued for this ordering")

type Repository struct {
	mu    sync.Mutex
	rows  map[string]clinical.Injury
	next  int
	clock time.Time
}

func NewRepository() *Repository {
	return &Repository{rows: make(map[string]clinical.Injury), clock: epoch}
}

func (r *Repository) List(_ context.Context, patientID string, query injury.Query) (domain.Page[clinical.Injury], error) {
	r.mu.Lock()
	defer r.mu.Unlock()

	sortKeys := query.Sort
	if len(sortKeys) == 0 {
		sortKeys = injury.Sorts()[:1]
	}

	matched := make([]clinical.Injury, 0, len(r.rows))
	...
	slices.SortFunc(matched, func(left, right clinical.Injury) int {
	...
	if query.Cursor != "" {
		...
		after := make([]clinical.Injury, 0, len(matched))
...
func (r *Repository) Get(_ context.Context, id string) (clinical.Injury, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Repository) Create(_ context.Context, entity clinical.Injury) (clinical.Injury, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	entity.ID = fmt.Sprintf("mkfakeinj%06d", r.next)
...
func (r *Repository) Update(_ context.Context, entity clinical.Injury, expectedVersion string) (clinical.Injury, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if current.Version != expectedVersion {
		return clinical.Injury{}, domain.ErrVersionMismatch
	}
...
func (r *Repository) byID(id string) (clinical.Injury, error) {
	row, exists := r.rows[id]
	if !exists {
		return clinical.Injury{}, fmt.Errorf("injurytest: no such record: %w", domain.ErrNotFound)
	}
...
func matches(row clinical.Injury, query injury.Query) bool {
	if len(query.Statuses) > 0 && !slices.Contains(query.Statuses, row.Status) {
		return false
	}

	if len(query.Severities) > 0 && !slices.Contains(query.Severities, row.Severity) {
		return false
	}
	...
	if len(query.Lateralities) > 0 && !slices.Contains(query.Lateralities, row.Laterality) {
		return false
	}
...
func sortValue(row clinical.Injury, field string) string {
	switch field {
	case injury.FieldName:
		return asciiLower(row.Name)
	case injury.FieldOccurredOn:
		return row.OccurredOn.String()
	case injury.FieldUpdated:
...
func compareValues(field string, left, right string, desc bool) int {
	if field == injury.FieldOccurredOn && (left == "" || right == "") {
		switch {
		case left == "" && right == "":
			return 0
		case left == "":
...
func compareRows(left, right clinical.Injury, sortKeys []domain.SortKey) int {
...
func encodeBoundary(row clinical.Injury, sortKeys []domain.SortKey) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("injurytest: sealing the boundary: %w", err)
	}
...
func compareToBoundary(row clinical.Injury, after boundary, sortKeys []domain.SortKey) int {
