...
func New(repository Repository, authorizer Authorizer) (*Service, error) {
	...
	if len(missing) > 0 {
		return nil, fmt.Errorf("procedure: the service is wired with no %s", joinWords(missing))
	}

	return &Service{repository: repository, authorizer: authorizer}, nil
}

func (s *Service) List(ctx context.Context, actor access.Actor, query Query) (domain.Page[clinical.Procedure], error) {
	if err := s.authorizePatient(ctx, actor, query.PatientID, access.PermView); err != nil {
		return domain.Page[clinical.Procedure]{}, err
	}
...
func (s *Service) Get(ctx context.Context, actor access.Actor, id string) (clinical.Procedure, error) {
...
func (s *Service) Create(ctx context.Context, actor access.Actor, draft clinical.Procedure) (clinical.Procedure, error) {
	if draft.PatientID == "" {
		var invalid domain.ValidationError
		invalid.Add(FieldPatient, domain.CodeRequired, "a procedure is filed against a person")

		return clinical.Procedure{}, invalid.OrNil()
	}
...
func (s *Service) Update(ctx context.Context, actor access.Actor, id, version string, patch Patch) (clinical.Procedure, error) {
...
func (s *Service) authorizePatient(ctx context.Context, actor access.Actor, patientID string, need access.Permission) error {
	...
	if !grant.Allows(need) {
		return fmt.Errorf("procedure: the authorization checkpoint granted nothing and refused nothing: %w", domain.ErrNotFound)
	}
