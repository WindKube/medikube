...
is removed and no gate is weakened.
 ...
 11. Technology Constraints: PocketBase's own `Settings()` store is CARVED OUT
     ...
     second config system MediGo chose; it is part of the platform Principle V
     says MediGo must not reimplement.
...
soft delete. New guidance, nothing removed or weakened.
  ...
  7. Technology Constraints: MediGo is SINGLE-INSTANCE BY CONSTRUCTION.
...
Amendments in 1.1.0:
  1. Technology Constraints: Go 1.26.5 -> Go 1.27. PocketBase v0.40.1's go.mod
     ...
     with "go.mod requires go >= 1.27"; go1.27.0 builds it clean. MediGo is
  ...
  2. Principle V: PocketBase no longer "owns realtime". Its native realtime is
     ...
     MediGo's realtime and a Go-side bridge translates record hooks into it.
  ...
  4. Principle VII: mandatory `Protected: true` on every file field plus
     MediGo-owned file routes, closing an unauthenticated-file-access hole that
...
MediGo is a self-hosted personal medical records application written in Go. It
...
embedded PocketBase. It is a reimagining, not a port: MediGo owes MediKeep no
API compatibility and is free to design a smaller, cleaner surface.

The data MediGo holds is among the most sensitive a person owns. Every
...
simplicity gate, MediGo becomes a demonstration of its dependency list rather
than a medical records application.

### II. Interfaces At Every Seam (SOLID)

MediGo is structured so that its domain logic never depends on PocketBase, on
...
- **Interface segregation.** Interfaces are declared by the CONSUMER package,
  ...
  normal; more than five requires justification in `plan.md`. MediGo MUST NOT
...
cadence. Confining it to an adapter layer is what makes MediGo testable
...
MediGo reads like Go written by someone who likes Go.
...
- `samber/ro` MUST NOT be a dependency either. Despite the name it is not a
  ...
  update. Principle I settles this: MediGo's realtime hub is a channel and a
...
database, authentication, file storage, and the superuser admin UI. MediGo does
not fight it and does not reimplement it. PocketBase does NOT own realtime —
see the realtime clause below.

- MediGo MUST NOT rebuild what PocketBase provides. Authentication,
...
- PocketBase's automatic record CRUD API — the
  ...
  MediGo's authentication mechanism.
...
- **Realtime is Datastar's, not PocketBase's.** PocketBase's native realtime
  ...
  cannot perform. Live updates are therefore delivered by a MediGo-owned bridge:
...
- PocketBase's JavaScript VM plugin (`jsvm`) MUST NOT be registered. MediGo
...
- MediGo ships as ONE binary with no cgo, with static assets embedded via
...
Diagnosing MediGo means reading one stream.

- `zerolog` is the ONLY logger for MediGo's own code. Structured JSON to
...
MediGo stores diagnoses, medications, lab results, and identities. Privacy is
...
- MediGo makes no outbound network request that the operator has not
...
- Files MUST be served exclusively from MediGo's own `/api/v1` routes, through
  ...
  it lands in logs, proxies and referrer headers. Because MediGo bypasses
...
- The PocketBase superuser admin UI SHIPS in production. Because a superuser
  ...
  allowlist configured is a misconfiguration, and MediGo MUST warn loudly at
...
- Soft delete applies to FILES ONLY. A deleted attachment is recoverable for a
  ...
  the interface and recorded in the audit trail. MediGo MUST NOT grow a
...
- The route list under test MUST be derived from the application itself — a
  `medigo routes` subcommand emitting the registered route inventory as JSON —
...
- The gate runs against a deterministically seeded instance brought up by a
  `medigo seed` subcommand. Authenticated routes are covered, using a seeded
  test user.
...
Every claim MediGo makes about itself is machine-checked, or it is not made.
...
house standard, and MediGo inherits it.
...
- **MediGo is single-instance by construction and MUST NOT be horizontally
...
- **Go 1.27.** Single static binary, no cgo — pure-Go SQLite arrives via
  ...
  `apis/`. Building on 1.26.5 fails outright. This makes MediGo the only project
  ...
  1.27's `encoding/json` v2 semantics in MediGo's own DTOs, which are not fully
...
- `caarlos0/env/v11` — the only configuration mechanism MediGo itself defines.
  ...
  MediGo chose; it is part of the platform Principle V forbids reimplementing.
  MediGo MUST NOT mirror those settings into environment variables, and MUST NOT
  invent its own UI for them. Where an unconfigured PocketBase setting would
  silently break a MediGo feature — SMTP for invitations being the live example
  — MediGo MUST surface that state and warn at boot rather than fail quietly.
- `github.com/getkin/kin-openapi` — constructs the OpenAPI document. It is a
  ...
  introspectable: the document is BUILT from MediGo's single declarative route
...
- `samber/do` — dependency injection for MediGo's own services. PocketBase's
...
**Deliberately out of scope.** MediGo does not implement Paperless-ngx sync,
Papra sync, OCR of lab documents, or migration of existing MediKeep data.
MediGo is greenfield. It MUST, however, be able to export a user's complete
...
- `Taskfile.yaml` is the entry point for every routine action, matching this
  ...
  lines.
- Commits follow Conventional Commits, scoped `medigo`.
...
- MediGo is a project inside a shared monorepo. Its integration points —
