...
| Path | What it is |
|---|---|
| `source/MediKube.dc.html` | The comp as imported, with the app-name strings updated. 21 screens driven by a `screen` state value. |
...
Open `source/MediKube.dc.html` in a browser. It loads `support.js` alongside React from the page
...
MediKube's front end is templ + Datastar + Tailwind. Nothing in this directory changes that.

## Docker

`medikube/design/` must be excluded from the monorepo build context — see `specs/DESIGN.md` §8.
